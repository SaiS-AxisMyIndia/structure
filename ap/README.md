...
own classes and keeps real tables in sync (`gg build`), a stateless,
...
`packages/gerogo`, `packages/pro-sql`, `packages/session`, `packages/tester`
and `packages/app-viewer` are **framework packages** (treat them like
dependencies/libraries — namespaces `Gerogo\`, `ProSql\`, `Session\`,
...
- [runner/ and Gerogo\Runner](#runner-and-gerogorunner)
- [manifest.php and module references](#manifestphp-and-module-references)
...
- [Entities & schema sync — `gg build`'s auto-migration](#entities--schema-sync--gg-builds-auto-migration)
...
- [gg — the CLI](#gg--the-cli)
- [Flavours: `-f`/`--flavour <name>`](#flavours--f--flavour-name)
...
├── gg                        # the CLI (php gg ...) — gg -v, gg start/stop, gg build [-c], gg clean, gg install [module]
├── mcp-server                  # an MCP dev-tool server wrapping gg for an AI assistant — see tools/mcp/README.md, NOT part of the app
├── tools/mcp/                   # McpServer.php (protocol), GerogoTools.php (the tools), list-routes.php — none of it PSR-4/autoloaded
├── manifest.php                    # the application manifest (like application.properties) — name/version/env + modules to boot
...
├── .env.local                  # APP_ENV=local, SERVICE/PORT/VERSION, DB_*, SESSION_* — the "local" flavour, the default
├── .env.production              # same shape, flavour "production" — loaded by `gg ... -f production`
├── .env.staging                  # same shape, flavour "staging" — loaded by `gg ... -f staging`
│                                 # these three are the ONLY flavours out of the box (Runner::STAGES is a
│                                 # documented default list, not enforced) — see "Flavours" below to add another
├── prologs.log                # gitignored — every crash, boot-time or request-time; see Gerogo\Log — unbounded, LOGS=false to disable
├── lib/                         # never web-reachable (see .htaccess)
│   ├── page.html                  # the base <!doctype html> layout Page::render() fills in (builder mode)
│   ├── default.html                # the {{status}}/{{reason}}/{{message}} template Page::failed() fills in — see "Using Page"
│   ├── controllers/                # this app's own HTML/Page-returning controllers ("Lib\Controllers\", #[PageController]) — /site, /site/about, /site/docs, /site/releases, /site/contact
...
├── runner/                     # never web-reachable — boot-time config, one file per concern, regenerated by `gg build`
│   ├── runner.php                 # boots Gerogo\Runner — loads .env.<flavour> + manifest.php + every sibling file here, once
...
│   ├── tester.php                  # { enabled }, consumed by TesterModule — TESTER_ENABLED in .env.<flavour>
│   ├── app_viewer.php              # { enabled }, consumed by AppViewerModule — APP_VIEWER_ENABLED in .env.<flavour>
│   ├── controllers.php             # this app's own controller list — auto-discovered by Gerogo\ClassDiscovery (every #[RestController]/#[PageController] under src/Controllers/ + lib/controllers/)
│   └── entities.php                # this app's own #[ProEntity] list — auto-discovered under src/Entities/, consumed by ProSqlModule::build()
├── storage/
│   ├── logs/                   # runtime output
│   ├── pids/                    # gitignored — <flavour>.pid, written by `gg start`, read by `gg stop`
│   ├── migrations/              # one timestamped .sql file per `gg build` run that touched the schema — see "Entities & schema sync"
...
└── packages/
    ├── gerogo/               # the WEB framework package ("Gerogo\") — a dependency, not app code
    │   ├── composer.json
    ...
    │   ├── PackageResolver.php    # resolves a manifest.php '@name' => 'version' entry to a packages/ Module
    │   ├── ClassDiscovery.php     # PSR-4-aware attribute scanner — the mechanism behind auto-discovered controllers.php/entities.php
    │   ├── Runner.php             # loads .env.<flavour> + manifest.php once; resolves modules once; compiles + caches routes once
    ...
    │   ├── Attributes/            # @RestController / @PageController / @GetMapping / .../ @Middleware / @Service / @BaseRoot
    │   └── Cli/                   # the gg CLI's command classes — see "gg — the CLI" below for what each does
    │
    ...
    └── app-viewer/              # the APPVIEWER package ("AppViewer\") — Tester's sibling, for Page-returning routes
        ...
        ├── AppViewerController.php  # GET /app-viewer (UI), GET /app-viewer/pages (JSON list + fields), POST .../request + .../render (re-invoke with a real, synthetic Request)
...
AppViewer" for why that split exists). [manifest.php](manifest.php) — at the project
...
disposable build artifact: `gg build` regenerates every file in it (see
"gg — the CLI"), so nothing here should ever be hand-edited and expected
to survive.

- **[runner/runner.php](runner/runner.php)** — "the configuration set for
  running". Loads Composer's autoloader, calls `Gerogo\Runner::boot($basePath)`
  (which resolves the right `.env` file for the current flavour,
  evaluates `manifest.php`, and requires every other file in this folder — all
...
- **[runner/controllers.php](runner/controllers.php)** — this app's own
  ...
  `#[RestController]` or `#[PageController]`, found by `Gerogo\ClassDiscovery`).
...
- **[runner/tester.php](runner/tester.php)** — `{ enabled }` for
  ...
  inside `TesterModule` (`TESTER_ENABLED` in `.env.<flavour>`) — see "Using
  Tester" below.
...
else should read `$_ENV`, `require manifest.php`, or `require` a `runner/*.php`
...
Runner::get('name');              // a key from manifest.php's returned array — e.g. 'gerogo'
...
every one of their controllers' `#[RestController]`/`#[PageController]`/
`#[GetMapping]`/`#[Middleware]` attributes into a plain route table
(`Runner::routes()`, via `Gerogo\RouteCompiler`) — Reflection only ever
runs here, never inside `Router::dispatch()`; the same compile step is
where the `Page`-return-type rule below gets enforced, once, rather than
checked on every dispatch. `Kernel` just hands the table to `Router`,
which matches a request's path/method against it and calls the
...
one**: in `manifest.php`'s `env: 'local'` (the default), `Runner::routes()`
...
after deploying a routing change, or run `gg build`/`gg build --clean`).

## manifest.php and module references

[manifest.php](manifest.php) is the one manifest — name/version/env, plus the
...
`version` reads `VERSION` from whichever `.env.<flavour>` file booted (same
pattern as `env`/`APP_ENV`) — set it there per flavour, not by editing
`manifest.php` itself. `gg -v`/`gg start` both print it via
`Runner::get('version')` — `gg start`'s own identity banner reads
`<SERVICE> v<VERSION>` followed by `env:`/`db:` lines.

A module entry is one of:

- **`'@name' => 'version'`** — always resolved from `packages/<name>`,
  never anywhere else. `Gerogo\PackageResolver` reads
  `packages/<name>/composer.json`, checks its `version` matches exactly
  (booting fails loudly on a mismatch — e.g. you bumped
  `packages/pro-sql/composer.json` to `1.1.0` but `manifest.php` still says
  ...
  Gerogo\Runner" above). A plain `Application::class` entry alongside these
...
spot you'd put `#[RestController]` (or `#[PageController]`). Each entry is either a plain
...
loaded via `gg build --production`/`gg start --staging`/etc.) —
...
## Entities & schema sync — `gg build`'s auto-migration
...
`gg build` figures out what the database needs and (depending on
...
`gg build` (via `ProSqlModule::build()` → `ProSql\Schema\SchemaBuilder`)
scans every class `runner/entities.php` lists (auto-discovered — see
"runner/ and Gerogo\Runner"), reads what each table actually looks like
right now (`SchemaInspector`, straight from `information_schema`), diffs
the two (`SchemaDiffer`), and — depending on `TABLE_WRITE` in
`.env.<flavour>` — either applies the difference or just reports it:
...
// runner/session.php
return [
    'secret' => $_ENV['SESSION_SECRET'] ?? 'change-me-in-.env.<flavour>',
...
- **`Session::resolve($token)`** — decodes + verifies an incoming compact
  ...
  expired.
- **`Session::response($packet)`** — merges an `Gerogo\Packet`'s data into
...
- **`Session::logout()`** — drops the current token, so the response that
  ...
  outstanding token at once, bump `SESSION_VERSION` in `.env.<flavour>` instead;
...
`Gerogo\Packet` builds a consistent `{ success, message, data }` body, and
it's the **only** shape `Gerogo\Response::json()` accepts — its signature is
...
`Gerogo\Page` is `Packet`'s counterpart for actual web pages instead of a
...
`new Page($request, 'HomePage')` is shorthand for
`(new Page())->view('HomePage')` — `lib/page/HomePage.php` is a plain PHP
file where you write the *entire* document yourself, doctype included
(`title()`/`body()`/`style()`/`script()` are ignored once a view is
set — there's no placeholder template to fill in anymore, the view file
*is* the page).

There's no separate "props" step — `$request` (once set here) is the
only thing `extract()`-ed into the view file's own scope as a real local
variable. A view pulls whatever it actually needs straight from
`$request` (the same `InputBag` getters a controller would use):

```php
<?php
use Gerogo\Page, Gerogo\Request;
/** @var Request $request */
$name = $request->query->getString('name', 'World'); // optional — 'World' if absent
?>
<h1>Hello, <?= Page::html($name) ?></h1>
```

For anything that isn't real request data — a value only a
constructor-injected service can produce — the controller ADDS it onto
`$request` before returning, via `InputBag`'s `addX()` methods (one per
`getX()`: `addString`/`addInt`/`addFloat`/`addBool`/`addArray`/
`addJson`/`addMail`/`addPassword`, each fluent) — the write counterpart
to validation, not a separate mechanism:
...
use Gerogo\Page, Gerogo\Request;
/** @var Request $request */
$posts = $request->body->getJson('posts', []); // read back the exact same way a real body field would be
?>
<ul>
...
invalid required field 400s before the view ever runs), then just
return the `Page`:
...
`Request::all()` hands back the whole request as one plain array —
`['query' => [...], 'body' => [...], 'params' => [...]]` — for anything
that wants it as one JSON-able value instead of reading `query`/`body`/
`params` individually (AppViewer's own Request panel uses this to echo
back exactly what it's about to invoke with — see "Using AppViewer"
below).

Either way, a validation failure — `InputBag`, or anything throwing
`PacketFailed` (an expired session via `#[Middleware(SessionMiddleware(...))]`,
your own controller code) — on a **`#[PageController]`** route renders as
a **styled HTML error page** instead of a raw JSON body: `Kernel::handle()`
checks `Request::$isPage` (set by `Router` the moment a `#[PageController]`
route matches) and reaches for `Page::failed($status, $message)` —
`400`/`401`/`403`/`404`/... all render with a real reason phrase, in the
same dark theme `CrashPage` uses for a boot-time failure, just carrying
the real status instead of always `503`. A `#[RestController]` route
failing the exact same way still gets the usual JSON `Packet` — nothing
about that path changes.

A path that matches **no route at all** — a genuine 404, nothing left
under `#[GetMapping]`/`#[PostMapping]`/... to even ask `isPage` from —
falls back to content negotiation instead: `Router::dispatch()` sets
`Request::$isPage` from `Request::wantsHtml()`, which checks the
client's own `Accept` header for `text/html`. A real browser's own
top-level navigation (typing a URL, clicking a broken link) always
sends that first, so a mistyped/removed page's 404 still renders as a
page, not raw JSON dropped into a browser tab; a plain XHR/fetch/`curl`
call doesn't send it (unless it explicitly asks for HTML too), so a
genuine REST API 404 is unaffected. A *matched* route's own failure
never reaches this fallback at all — its `isPage` was already decided,
definitively, by which attribute it carries.

`Page::failed()` fills in **[lib/default.html](lib/default.html)**'s
`{{status}}`/`{{reason}}`/`{{message}}` placeholders — the exact same
customize-by-editing-a-file convention `lib/page.html` uses above; falls
back to an identical built-in template if that file is ever missing or
deleted. It's a complete document on its own (doctype included), not a
`{{body}}` fragment — `Page::failed()` bypasses `view()`/the builder
mode entirely, so editing `lib/page.html` has no effect on it.
...
action returning `new Page($request, '...')`, one matching file per
controller in `lib/page/`. `HomeController` in particular adds its
`PostService` result onto the request first (`$request->body->addJson('posts', ...)`
— InputBag's write counterpart to `getJson()`) so `HomePage.php` reads
`$posts` back the exact same way it'd read any other body field, and the
page's own JS `POST`s a new one to `App\Controllers\HomeController::createPost()`
(`/ap/v1/gerogo/posts`) and appends the result to the DOM — a genuine
...
Every class in `lib/controllers/` carries `#[PageController(prefix: ...)]`
instead of `#[RestController(...)]` — the framework-enforced counterpart
for HTML-rendering routes (see "Adding a new endpoint" for the full
attribute). `RouteCompiler` rejects a `#[RestController]` action that
declares `Page` as its return type, and rejects a `#[PageController]`
action that DOESN'T — so "which half a controller belongs to" is a
compile-time guarantee, not just a directory convention, and it's what
lets `Tester` and `AppViewer` each show only their own half reliably:
see "Using AppViewer" below for how they tell the two apart.

## Validating input with InputBag

`$request->query`, `$request->body`, and `$request->params` (route
placeholders like `/{id}`) are all an `Gerogo\InputBag` — plain array
...
### Writing back — `addX()`, one per `getX()`

Every getter has a fluent write counterpart — `addString`/`addInt`/
`addFloat`/`addBool`/`addArray`/`addJson`/`addMail`/`addPassword` — for
putting something INTO an `InputBag` rather than reading it out. Not for
real request data (that's already there); for a value only your own code
can produce, that something downstream should still read back through
the exact same `getX()` call — a `#[PageController]` action handing a
service's result to its own view, without a separate props/DI mechanism
(see "Using Page" above):

```php
$request->body->addJson('posts', $this->postService->all());
// -> lib/page/HomePage.php: $request->body->getJson('posts', [])
```

`RouteCompiler::fieldsOf()` picks up an `addX()` call the same way it
picks up a `getX()` one, tagged `kind: 'add'` instead of `'get'` (always
`required: false` — there's nothing to require, the controller provides
it). Tester and AppViewer both use this to tell the two apart: a `'get'`
field renders as a real, fillable input (what a caller supplies); a
`'add'` field renders as a plain read-only mention — just the variable
name and type, no input box, since nothing a caller types would ever
reach it.
...
`TESTER_ENABLED` in `.env.<flavour>`) — **enabled by default**, since it's meant as a
...
whose controller method returns `Page` — click one to see it live, fill
in its **Request** panel (the exact same field UI Tester renders — one
labeled input per query/body field the action actually validates, plus
any path params), and **Invoke** to re-run it against a real,
synthetic `Request` built from what you typed, right there in the
browser.

Both tools use the exact same check to decide what's "a page": whether
the controller method's declared return type is `Page` (via
`Gerogo\Page::isReturnedBy()`, checked by Reflection). That's reliable on
its own — not a heuristic — because `RouteCompiler` enforces it at
compile time: a `#[PageController]` action always declares `Page`, a
`#[RestController]` action never does (see "Adding a new endpoint"),
so `Tester` and `AppViewer` can't ever disagree with which attribute a
controller actually carries. `Tester` uses this check to **exclude**
page routes from its own list; `AppViewer` uses the identical check to
**include** only them, so a route always shows up in exactly one
explorer, never both, without either one having to know the other's
route names.

```php
// AppViewerController — driven by the same compiled route table Tester and Router use
GET  /app-viewer/pages    // every Page-returning route (route.fields included), minus /app-viewer's own
POST /app-viewer/request   // echoes back the synthetic Request {query, body, params} it would invoke with — no side effects
POST /app-viewer/render    // actually re-invokes with it, returns the fully rendered HTML
```

`request`/`render` both take `?controller=&action=` (which route) plus a
JSON body shaped `{query: {...}, body: {...}, params: {...}}` — exactly
what the UI's Request panel builds from whatever you typed, the same
`buildFieldObject()`/blank-omission rules Tester's Send button uses.
`Gerogo\Request`'s constructor accepts these as optional overrides (see
its own docblock) — the target action gets a REAL `Request` carrying
your values, so `$request->query->getX(...)`/`body`/`params` inside it
behave exactly as they would for a real visitor, not AppViewer's own
`/app-viewer/render` request forwarded blind. `render` works for a
`Page` built either way (`view()` or the `title()`/`body()` template
builder) since it renders the actual `Page` object the action returned.
Since re-invoking a controller action means genuinely calling it again
— real side effects included, not a cached snapshot — a page whose
action writes something on every call (a counter, a log line) will do
that again each time you Invoke it here, the same as a real request
would.

Controlled by [runner/app_viewer.php](runner/app_viewer.php)
(`Runner::get('app_viewer')['enabled']`, `APP_VIEWER_ENABLED` in
`.env.<flavour>`) — same enabled-by-default convention as `TESTER_ENABLED`,
...
anyone who visits it. `gg start`/`gg -v` print both tools' URLs
...
1. Add a method to a controller in `src/Controllers/` (or a new controller
   ...
   on the class or method if it needs to run through one. **A
   `#[RestController]` action must never declare `Page` as its return
   type** — `RouteCompiler` rejects it at compile time; return `Packet`
   (or `array`, or nothing) instead. Building an HTML page? Use
   `#[PageController(prefix: '/...')]` in `lib/controllers/` instead (same
   shape, same mapping attributes) — its actions must ALWAYS declare
   `Page` as their return type, enforced the same way, in the other
   direction. A class may carry one of the two, never both.
2. Run `gg build` (or `gg build --clean`) so `runner/controllers.php`
   ...
   fresh every request in `env: local` anyway. A build that violates
   either return-type rule above fails loudly right here, not at request
   time.
...
by `gg build` — see "runner/ and Gerogo\Runner" above. For a new feature
area, either add its controllers there, or create a class extending
`Gerogo\Module` and list it in `manifest.php`'s `modules` array — as
...
For a new middleware, implement `Gerogo\MiddlewareInterface` and
reference its class-string in a controller's `#[Middleware(...)]`.

## gg — the CLI

`php gg <command>` (or `./gg <command>` — it's executable) is this
...
`Gerogo\Runner` exactly the way `index.php` does — same module resolution,
same route-caching rules — so what it reports is what a real request
would actually get, not a separate config path.

```bash
php gg -v                          # app + every packages/* package's name and version
php gg start                       # clean + rebuild, then start PHP's built-in server at 127.0.0.1:<PORT in .env.<flavour>> (prints /tester, /app-viewer if enabled)
php gg start 8081                  # ...same, but on 127.0.0.1:8081
php gg stop                        # stop the ONE server a matching `gg start` began, wherever it's actually running
php gg build                       # regenerate runner/ in place, force-compile + cache routes, run every module's build() hook
php gg build -c, --clean           # delete the whole runner/ folder first, then build as above
php gg clean                       # delete the whole runner/ folder (and route cache) — no rebuild
php gg build -f local              # ...any of the above against .env.local instead of the real APP_ENV env var / 'local' default
php gg build -f production         # ...or against .env.production
php gg build --flavour=staging     # ...or .env.staging (--flavour=<name> works the same as -f <name>)
php gg start -f production         # start against .env.production, on its own PORT
php gg stop  -f production         # stop it, from any other process
php gg install                     # no module (defaults to `gerogo`): validates every manifest.php module reference actually resolves
php gg install gerogo 1.0.0       # ...and that manifest.php's own 'version' equals 1.0.0
php gg install pro-sql             # any other module: shows packages/pro-sql's version + whether/how manifest.php references it
php gg install pro-sql 1.0.0       # ...and validates packages/pro-sql's composer.json version equals 1.0.0
```
...
`1`) rather than ever rewriting `manifest.php` or a package's `composer.json`
...
right before you switch `manifest.php`'s `env` away from `local` — see
...
`Gerogo\Cli\Command` implementation under `packages/gerogo/Cli/` and
listing it in `Application::COMMANDS`.

## Flavours: `-f`/`--flavour <name>`

In this framework "env" and "flavour" are the same thing — there's only
one axis, not two. `-f <name>` (or `--flavour <name>`, or
`--flavour=<name>`) picks which `.env.<name>` file `gg` boots from —
`local`, `production`, `staging` (`Runner::STAGES` — a documented
default list, not an enforced one), or any other name a real `APP_ENV`
process env var happens to carry. No flag at all falls back to a real
`APP_ENV` env var, else `'local'` (see `Runner::boot()`).

A flavour is nothing more than "boot from a different `.env` file, on a
different `PORT`" — there's no separate lifecycle, daemon mode, or
per-flavour file naming scheme layered on top; every flavour is just
`.env.<name>`, exactly like `.env.local`/`.env.production`/`.env.staging`
already are (see `Runner::envFilePath()`). Every `.env.*` file must
declare its own matching `APP_ENV=<name>` line — `Runner::get('env')`
reflects whatever a file actually says, not just whichever `-f` flag was
passed.

Two env vars every one of these files carries: `SERVICE` (a display
name, shown in `gg start`'s own banner) and `PORT` (what `gg start`
binds to by default, when no explicit `host:port` is given).

`gg stop` never needs to be the same process, or even the same
terminal, that ran `gg start` — the server `gg start` spawns writes
its own PID to `storage/pids/<flavour>.pid` the moment it binds (see
[AppProcess](packages/gerogo/Cli/AppProcess.php)), and `gg stop`
computes that exact same filename independently, from the exact same
`-f` flag, to find and signal it.
...
[Gerogo\CrashPage](packages/gerogo/CrashPage.php) is what a boot failure
...
`Packet` instead (or, for a `#[PageController]` route, a styled HTML
error page via `Page::failed()` — see "Using Page" above), and THAT
still keeps the real message outside `env: local` — but both paths log
through the same place either way.

Either way, [Gerogo\Log::crash()](packages/gerogo/Log.php) appends a
...
log). Controlled by `LOGS` in `.env.<flavour>` — set `LOGS=false` to turn
logging off entirely; defaults to on.

## MCP dev-tool server

[tools/mcp/](tools/mcp/README.md) — an MCP server wrapping `gg` (plus
...
part of the app itself: nothing here is booted by `manifest.php`, resolved
...
curl -X POST localhost:7070/ap/v1/users \
  -H 'Content-Type: application/json' -d '{"mail":"test@gerogo.com","password":"secret"}'
