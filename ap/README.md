...
│   ├── runner.php                 # boots ApiPro\Runner — loads .env.<flavour> + app.php + every sibling file here, once
...
│   ├── controllers.php             # this app's own controller list — auto-discovered by ApiPro\ClassDiscovery (every #[RestController]/#[PageController] under src/Controllers/ + lib/controllers/)
...
- **[runner/controllers.php](runner/controllers.php)** — this app's own
  ...
  `#[RestController]` or `#[PageController]`, found by `ApiPro\ClassDiscovery`).
...
(`Runner::routes()`, via `ApiPro\RouteCompiler`) — Reflection only ever
...
```php
<?php
use ApiPro\Page, ApiPro\Request;
/** @var Request $request */
...
`ApiPro\Page::isReturnedBy()`, checked by Reflection). That's reliable on
...
`ApiPro\Request`'s constructor accepts these as optional overrides (see
...
[AppProcess](packages/api-pro/Cli/AppProcess.php)), and `apc stop`
