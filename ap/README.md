...
├── .env.production              # same shape, flavour "production" — loaded by `apc ... -f production`
├── .env.staging                  # same shape, flavour "staging" — loaded by `apc ... -f staging`
...
│   ├── runner.php                 # boots Gerogo\Runner — loads .env.<flavour> + app.php + every sibling file here, once
...
├── storage/
│   ├── logs/                   # runtime output
│   ├── pids/                    # gitignored — <flavour>.pid, written by `apc start`, read by `apc stop`
...
└── packages/
    ...
    │   ├── Runner.php             # loads .env.<flavour> + app.php once; resolves modules once; compiles + caches routes once
...
Runner::get('name');              // a key from app.php's returned array — e.g. 'gerogo'
...
`app.php` itself. `apc -v`/`apc start` both print it via
`Runner::get('version')` — `apc start`'s own identity banner reads
...
by `apc build` — see "runner/ and Gerogo\Runner" above. For a new feature
area, either add its controllers there, or create a class extending
`Gerogo\Module` and list it in `app.php`'s `modules` array — as
...
php apc start                       # clean + rebuild, then start PHP's built-in server at 127.0.0.1:<PORT in .env.<flavour>> (prints /tester, /app-viewer if enabled)
...
php apc build -f local              # ...any of the above against .env.local instead of the real APP_ENV env var / 'local' default
php apc build -f production         # ...or against .env.production
php apc build --flavour=staging     # ...or .env.staging (--flavour=<name> works the same as -f <name>)
php apc start -f production         # start against .env.production, on its own PORT
php apc stop  -f production         # stop it, from any other process
php apc install                     # no module (defaults to `gerogo`): validates every app.php module reference actually resolves
php apc install gerogo 1.0.0       # ...and that app.php's own 'version' equals 1.0.0
...
`--flavour=<name>`) picks which `.env.<name>` file `apc` boots from —
...
terminal, that ran `apc start` — the server `apc start` spawns writes
its own PID to `storage/pids/<flavour>.pid` the moment it binds (see
[AppProcess](packages/gerogo/Cli/AppProcess.php)), and `apc stop`
