  ...
  return (
    <View style={styles.container}>
      <View style={styles.buttonWrap}>
        <Pressable
          // Intentionally not using Pressable's `disabled` prop here: a
          // disabled Pressable doesn't claim the touch responder, so while
          // this button is "disabled" a tap on it falls through to the
          // notification card underneath and triggers its navigation
          // instead. Keeping it enabled and no-op'ing inside the handler
          // ensures this button always swallows its own taps.
          onPress={() => {
            if (player.isLoading || player.error) {return;}
            player.toggle();
          }}
          style={[styles.playButton, (player.isLoading || !!player.error) && styles.playButtonDisabled]}
        >
