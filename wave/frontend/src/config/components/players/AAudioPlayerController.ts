...
export function useAAudioPlayerController(source: string, isActive = true): AAudioPlayerController {
  ...
  // `soundRef.current` only gets set once `new Sound(...)`'s async load
  // callback fires. A second `play()` call before that resolves (a fast
  // double-tap, or - on web - a click landing before React's re-render
  // flips the "loading" state the button checks) would otherwise still see
  // `soundRef.current` as null and construct a second, independent Sound
  // for the same source - two overlapping playbacks of one item. This ref
  // flips synchronously the instant creation starts, so a concurrent call
  // sees it immediately, with no render/state round-trip in between.
  ...
  const startPlayback = useCallback(() => {
    ...
    // startProgressTimer is a plain function redefined every render (not
    // itself memoized) - depending on it here would change startPlayback's
    // own identity every render too.
