...
export function useLoadingController(initialLoading = true): LoadingController {
  ...
  const setLoading = (value: boolean) => {
    ...
    // Only clear a stale error when a *new* load is starting - not when
    // turning loading off. Every controller's load() follows the same
    // `try { ... } catch { setError(...) } finally { setLoading(false) }`
    // shape (see SettingsSController.ts and friends), and that finally
    // block runs after setError too. Clearing unconditionally here wiped
    // the error setError had just set back out on every single one of
    // them, so a real fetch failure rendered nothing at all instead of the
    // intended error+retry UI - LoadingView's `errorMessage ? ... : body`
    // branch fell through to `body`, which is null/empty since the load
    // never succeeded.
    if (value) {
