...
export const StringFormatter = {
  ...
  url(text: string | null | undefined): string {
    if (!text || typeof text !== 'string') {return '';}

    if (text.startsWith('http://') || text.startsWith('https://')) {
      return text;
    }
    return Flavour.getImageBaseUrl() + text;
  },

  findByLabel(text: string, options: { label: string; value: string }[], caseSensitive: boolean = false): string | undefined {
    const value = caseSensitive ? text : text.toLowerCase();
    const label = (option: { label: string }) => (caseSensitive ? option.label : option.label.toLowerCase());
    return options.find(option => value.includes(label(option)))?.value;
  },
};
